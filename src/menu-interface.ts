...
export class MenuInterface implements IMenuInterface {
  ...
  invertColors(destroy: boolean) {
    if (typeof this._acc.stateValues.html.backgroundColor === "undefined")
      this._acc.stateValues.html.backgroundColor = getComputedStyle(
        this._acc.html
      ).backgroundColor;
    ...
    if (destroy) {
      ...
      this._acc.resetIfDefined(
        this._acc.stateValues.html.color,
        this._acc.html.style,
        "color"
      );
    ...
    if (
      this._acc.stateValues.invertColors &&
  ...
  grayHues(destroy: boolean) {
    ...
    if (destroy) {
      ...
      this._acc.onChange(true);
      this._acc.resetIfDefined(
        this._acc.stateValues.html.filter,
        this._acc.html.style,
        "filter"
      );
      this._acc.resetIfDefined(
  ...
  textToSpeech(destroy: boolean) {
    ...
    const tSpeechList = document.querySelector(
      '._access-menu [data-access-action="textToSpeech"]'
    );
  ...
  speechToText(destroy: boolean) {
    ...
    const sTextList = document.querySelector(
      '._access-menu [data-access-action="speechToText"]'
    );
    if (!sTextList) return;
    ...
    let remove = () => {
      ...
      for (let i = 0; i < inputs.length; i++) {
        inputs[i].removeEventListener(
          "focus",
          this._acc.listen.bind(this._acc),
          false
        );
  ...
  disableAnimations(destroy: boolean) {
    ...
    const remove = () => {
      if (!this._acc.options.suppressDomInjection)
        document
          .querySelector(
            '._access-menu [data-access-action="disableAnimations"]'
          )
          .classList.remove("active");
  ...
  customFunctions(destroy?: boolean, button?: HTMLElement) {
    if (!button) return;
    const cf =
      this._acc.options.customFunctions[
        parseInt(button.getAttribute("data-access-custom-index"))
      ];
